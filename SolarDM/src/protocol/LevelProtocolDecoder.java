...
public class LevelProtocolDecoder extends BaseProtocolDecoder{
	 ...
	 * Initialize
	 */
	public LevelProtocolDecoder(ServerManager serverManager) {
		super(serverManager);
	}
	...
	private Position decodeNormalMessage(ChannelBuffer buf, Channel channel)
		...
		try {
			...
			if (!pattern.matcher(imeiNo).matches()) {
				...
				ChannelBufferTools.updateHistoryForLevel(session,
